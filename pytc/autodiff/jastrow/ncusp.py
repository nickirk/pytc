...
def make_ncusp_jastrow(mol, n_radial=1000):
    """
    Creates a Nuclear Cusp Jastrow factor.
    
    Args:
        mol: Molecule object (pytc compatible)
        n_radial: Number of radial grid points for spline
    ...
    # --- Helper functions captured in closure ---

    def _eval_mo_at_r(nucleus_idx, r):
        ...
        mask = jnp.arange(n_nuclei) == nucleus_idx
    ...
    def _get_phi_s_derivatives(nucleus_idx, r):
    ...
    def _compute_X_values(Z_idx, rc, X4):
        """Compute all X values given rc and X4."""
        Z = unique_Z[Z_idx]
        # Use pre-computed mapping instead of jnp.where
        nucleus_idx = Z_idx_to_nucleus[Z_idx]
        
        phi_rc_vals = _get_phi_s_derivatives(nucleus_idx, rc)
    ...
    def _compute_alpha_coeffs(Z, rc, X_vals):
    ...
    def _eval_poly(r, coeffs):
        ...
        if r.ndim == 0:
             terms = coeffs * (r**powers)
             return jnp.sum(terms)
        else:
             # r is (N,)
             # Broadcast to (N, 5)
             r_pow = r[..., None] ** powers[None, :] 
             terms = coeffs[None, :] * r_pow
             return jnp.sum(terms, axis=-1)

    def _clip_params(params):
        """Clip parameters to valid ranges for each nucleus type."""
        return {
            'rc': jnp.clip(params['rc'], rc_range[:,0], rc_range[:,1]),
            'X4': jnp.clip(params['X4'], X4_range[:,0], X4_range[:,1])
        }

    # --- Main init and apply functions ---

    def init() -> Mapping[str, Any]:
        """Initialize parameter dictionary structure."""
        # Initialize parameters for each unique nuclear type
        params = {
            'rc': jnp.array([1.0/float(Z) for Z in unique_Z]), 
            'X4': jnp.zeros(n_types),
        }
        
        # Initialize X4 and compute alpha coefficients for each nucleus type
        for Z_idx, Z in enumerate(unique_Z):
            # Find first nucleus of this type
            nucleus_idx = int(Z_idx_to_nucleus[Z_idx])
            phi_0 = _eval_mo_at_r(nucleus_idx, 0.0)
    ...
    def apply(r_ae, params, nspins=None):
        """
        Compute nuclear cusp correction.
        
        Args:
            r_ae: Electron-nucleus distances (nelec, natom)
            params: Dictionary of parameters
            nspins: Tuple of (n_alpha, n_beta) - unused but kept for interface compatibility
        """
        # Clip parameters before use
        params = _clip_params(params)

        # Compute polynomial coefficients from current params
        poly_coeffs = jnp.zeros((n_types, 5))
        for Z_idx, Z in enumerate(unique_Z):
            rc = params['rc'][Z_idx]
            X4 = params['X4'][Z_idx]
        ...
        def compute_nucleus_contribution(nucleus_idx):
            # Get distance from all electrons to this nucleus
            # r_ae is (nelec, natom)
            r = r_ae[:, nucleus_idx] # (nelec,)
            
            # Use array indexing instead of dictionary lookup
            Z = charges[nucleus_idx]
            Z_idx = Z_to_idx[Z.astype(jnp.int32)]
            rc = params['rc'][Z_idx]
            ...
            poly_val = jnp.where(r<=rc, _eval_poly(r, coeffs), 0.0)
            phi_cusp = jnp.exp(poly_val)
            
            # Get φ_s value with numerical safeguard
            phi_s = jnp.where(r<=rc, _eval_mo_at_r(nucleus_idx, r), 1.0)
