...
class TestConvertXToRankMajor(unittest.TestCase):
    ...
    def test_whole_store_conversion(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with h5py.File(dst_path, "r") as fh:
                ...
                # X is preserved verbatim (legacy consumers); the twin is
                # appended alongside with layout + provenance stamp.
                ...
                self.assertIn("x_source_stamp", fh["X_rm"].attrs)
    ...
    def test_stale_twin_is_rebuilt(self):
        # X rewritten after the twin was built: the provenance stamp must
        # catch it and rebuild, never serve stale content.
        rng = np.random.default_rng(7)
        x = rng.normal(size=(7, 7, 8))
        x2 = rng.normal(size=(7, 7, 8))
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "store.h5")
            with h5py.File(path, "w") as fh:
                fh.create_dataset("X", data=x, dtype="f8")
            ...
            with h5py.File(path, "r+") as fh:
                del fh["X"]
                fh.create_dataset("X", data=x2, dtype="f8")
            add_rank_major(path, row_block=3)
            with h5py.File(path, "r") as fh:
                np.testing.assert_array_equal(np.asarray(fh["X_rm"]),
                                              x2.transpose(2, 0, 1))

    def test_single_entry_mutation_invalidates_twin(self):
        # Mutating one unsampled entry (no full rewrite) must also be
        # caught: the stamp covers every slab, no sampling shortcuts.
        rng = np.random.default_rng(7)
        x = rng.normal(size=(7, 7, 8))
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "store.h5")
