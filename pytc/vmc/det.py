...
class SlaterDeterminant:
    ...
    def grad(self, coords):
        ...
        ao_vals, ao_grads = numint.eval_ao(self.mol, coords, deriv=1)
        
        # Split into up and down electron parts
        ao_grads_up = ao_grads[:self.n_up]      # shape (n_up, nAOs, 3)
        ao_grads_down = ao_grads[self.n_up:]    # shape (n_down, nAOs, 3)
        ...
        grad_up = np.zeros((self.n_up, self.n_up, 3))
        grad_down = np.zeros((self.n_down, self.n_down, 3))
        
        # Handle each spatial direction
        for d in range(3):
            grad_up[..., d] = ao_grads_up[..., d] @ self.mo_coeff_alpha[:, :self.n_up]
            grad_down[..., d] = ao_grads_down[..., d] @ self.mo_coeff_beta[:, :self.n_down]
    ...
    def laplacian(self, coords):
        ...
        ao_data = numint.eval_ao(self.mol, coords, deriv=2)
        ao_vals, ao_grads, ao_lapls = ao_data
        
        # Split laplacians into up and down electron parts
        # Note: PySCF returns laplacian as sum of second derivatives
        ao_lapls_up = ao_lapls[:self.n_up]      # shape (n_up, nAOs)
        ao_lapls_down = ao_lapls[self.n_up:]    # shape (n_down, nAOs)
        
        # Contract with MO coefficients to get laplacians of molecular orbitals
        lapl_up = ao_lapls_up @ self.mo_coeff_alpha[:, :self.n_up]    # shape (n_up, n_up)
        lapl_down = ao_lapls_down @ self.mo_coeff_beta[:, :self.n_down]  # shape (n_down, n_down)
    ...
    def matrix(self, coords):
        ...
        ao_vals_all = numint.eval_ao(coords, deriv=0)
    
        # Partition the AO values by spin
        ao_up = ao_vals_all[:self.n_up]      # shape (n_up, nAOs)
        ao_down = ao_vals_all[self.n_up:]    # shape (n_down, nAOs)
        ...
        slater_up = mat_up[:, :self.n_up]        # shape (n_up, n_up)
        slater_down = mat_down[:, :self.n_down]  # shape (n_down, n_down)
    ...
    def value(self, coords):
        ...
        """
        slater_up, slater_down = self.compute_slater_matrices(coords)
    ...
    def update(self, e_idx, new_pos):
        ...
        ao_new = self._eval_AO(new_pos.reshape(1, 3)).squeeze(axis=0)
        ...
        n_spin = self.n_up if is_alpha else self.n_down
        ...
        local_idx = e_idx if is_alpha else e_idx - self.n_up
