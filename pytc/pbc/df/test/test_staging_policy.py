...
class TestStageEtaRecomputeTile(unittest.TestCase):
    ...
    def test_matches_direct_build_pi_eta(self):
        ...
        Pi_recompute, eta_recompute = stage_eta_recompute_tile(
            X, ao_block_source, mesh_obj.phase, mesh_obj.neg
        )
        ...
        Pi_direct, eta_direct = build_pi_eta(X, ao_full, mesh_obj.phase, mesh_obj.neg)
    ...
    def test_q_slice_applied_after_build(self):
        ...
        Pi_full, eta_full = stage_eta_recompute_tile(X, ao_block_source, mesh_obj.phase, mesh_obj.neg)
        Pi_sliced, eta_sliced = stage_eta_recompute_tile(
            X, ao_block_source, mesh_obj.phase, mesh_obj.neg, q_slice=0
        )
