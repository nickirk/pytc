...
2. Split panel measurement into `panel_dense` and `panel_oracle`; each builds
   and times one selector only. Move their equality check to an untimed test.
3. Add `selection_mode` to `ISDFDF` and return mode, candidate rule/count and
   indices, pivots, realized rank, AO calls, AO grid points, and cache bytes.
