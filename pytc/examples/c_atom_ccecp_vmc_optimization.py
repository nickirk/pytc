"""End-to-end VMC optimization for the C atom (³P) with ccECP / cc-pVTZ.

Open-shell UHF reference; ground-state ³P (2s² 2p², S = 1, two unpaired 2p
electrons).  Used as a building block for the CO atomization energy at
ccECP/cc-pVTZ.

Basis: ccECP / cc-pVTZ.  ccECP large-core: [He] core removed, 4 valence
electrons; n_alpha = 3, n_beta = 1.

Trial wavefunction: JSD = single Slater × (NuclearCusp + BoysHandy).
NuclearCusp is gated off at the C nucleus by the ECP-aware mask.

Usage:
    python -m pytc.examples.c_atom_ccecp_vmc_optimization
"""
...
def build_c(basis: str = "ccecp-cc-pvtz"):
    mol = gto.M(
        atom="C 0 0 0",
        basis=basis,
        ecp="ccecp",
        spin=2,  # 2S = 2, ³P ground state, two unpaired 2p electrons
        unit="Bohr",
        verbose=0,
    )
    mf = scf.UHF(mol)
...
def main(
    ...
    print("=" * 70)
    print(f"C atom (³P) / ccECP / {basis} — VMC end-to-end optimization")
    print("=" * 70)
    mol, mf = build_c(basis)
    ...
    print()
    print("=" * 70)
    print(f"C atom / ccECP / {basis} VMC results")
    print("=" * 70)
