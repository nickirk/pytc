"""End-to-end VMC optimization for the O atom (³P) with ccECP / cc-pVTZ.
...
Basis: ccECP / cc-pVTZ.  ccECP large-core: [He] core removed, 6 valence
electrons; n_alpha = 4, n_beta = 2.

Trial wavefunction: JSD = single Slater × (NuclearCusp + BoysHandy).
NuclearCusp is gated off at the O nucleus by the ECP-aware mask.

Usage:
    python -m pytc.examples.o_atom_ccecp_vmc_optimization
"""
...
def build_o(basis: str = "ccecp-cc-pvtz"):
    mol = gto.M(
        atom="O 0 0 0",
        basis=basis,
        ecp="ccecp",
...
def main(
    ...
    print("=" * 70)
    print(f"O atom (³P) / ccECP / {basis} — VMC end-to-end optimization")
    print("=" * 70)
    mol, mf = build_o(basis)
    ...
    print()
    print("=" * 70)
    print(f"O atom / ccECP / {basis} VMC results")
    print("=" * 70)
