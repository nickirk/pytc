"""End-to-end VMC optimization for CO with the ccECP pseudopotential.

Motivation: provide a small-molecule VMC datapoint to anchor future
xTC+ECP validation against Simula, Alavi et al. arXiv:2412.05885v1
(Dec 2024).  Simula et al. report xTC-CCSD(T) atomization energies for
CO/N2/H2O/F2/etc. with eCEPP/ccECP at AVTZ/AVQZ and demonstrate chemical
accuracy via PP-2 (second-order BCH) commutator treatment.

System: CO (¹Σ⁺) at experimental bond length r = 2.132 Bohr
(≈ 1.12822 Å), closed-shell singlet, RHF.

Basis: ccECP / cc-pVTZ.  ccECP large-core: [He] core removed on each
atom.  C contributes 4 valence electrons, O contributes 6 → 10 total.
L_max_NL = 1 (s + p projectors).

Trial wavefunction: JSD = single Slater × (NuclearCusp + BoysHandy).
NuclearCusp is gated off at both C and O nuclei by the ECP-aware mask.

Usage:
    python -m pytc.examples.co_ccecp_vmc_optimization
"""
...
# Experimental bond length r_CO = 1.12822 Å = 2.132 Bohr.
BOND_LENGTH_BOHR = 2.132


def build_co(basis: str = "ccecp-cc-pvtz"):
    mol = gto.M(
        atom=f"C 0 0 0; O 0 0 {BOND_LENGTH_BOHR}",
        basis=basis,
        ecp="ccecp",
        spin=0,
        unit="Bohr",
...
def main(
    ...
    print("=" * 70)
    print(f"CO (¹Σ⁺) / ccECP / {basis} — VMC end-to-end optimization")
    print("=" * 70)
    mol, mf = build_co(basis)
    hf_energy = float(mf.e_tot)
    print(f"HF/RHF/ccECP/{basis} = {hf_energy:.6f} Ha")
    print(f"r_CO = {BOND_LENGTH_BOHR} Bohr (experimental)")
    ...
    print()
    print("=" * 70)
    print(f"CO / ccECP / {basis} VMC results")
    print("=" * 70)
