...
class XTC(TC):
    ...
    def isdf(self, n_rank=None):
        ...
        # Get orbital values and gradients on grid
        mo_values, mo_grad = self._eval_basis_on_grid()
        rho = mo_values
        
        # Prepare paired densities
        rho_paired = np.einsum('in,jn->ijn', rho, rho).reshape(-1, len(self.weights))
        rho_grad_paired = np.einsum('ind,jn->ijnd', mo_grad, rho).reshape(-1, len(self.weights), 3)
        
        # Take norm of gradient's spatial components
        rho_grad_normed = np.linalg.norm(rho_grad_paired, axis=-1)
        ...
        C_rho, xi_rho, C_grad, xi_grad, pivots = isdf_decompose_multi(
            rho_paired, rho_grad_normed, n_rank, n_rank
        )
        ...
        return {
            ...
            'rho_grad_normed': rho_grad_normed
        }
